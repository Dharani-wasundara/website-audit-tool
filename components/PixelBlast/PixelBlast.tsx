...
type ReinitConfig = { antialias: boolean; liquid: boolean; noiseAmount: number };

/** Uniforms object passed to the fullscreen `ShaderMaterial` (matches fragment shader). */
...
/** Lives for the lifetime of one WebGL init (recreated when antialias / liquid / noiseAmount change). */
type PixelBlastRuntime = {
  ...
  /** Latest rAF id + stop flag — rAF id must update every frame or cleanup cannot cancel the loop. */
  anim: { raf: number; disposed: boolean };
  disposeInputs?: () => void;
  quad: THREE.Mesh;
  timeOffset: number;
...
const createTouchTexture = () => {
  ...
  const drawPoint = (p: TouchTrailPoint) => {
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        ...
        t.anim.disposed = true;
        cancelAnimationFrame(t.anim.raf);
        t.disposeInputs?.();
      ...
      // Some GPUs/browsers return null from getShaderInfoLog/getProgramInfoLog; three.js calls .trim() on it and throws.
      ...
      const anim = { raf: 0, disposed: false };
      const animate = () => {
        ...
        if (liquidEffect) {
          ...
          if (uTimeUniform) uTimeUniform.value = globalTime;
        }
        if (noiseEffect) {
          const uTimeUniform = noiseEffect.uniforms.get('uTime');
          if (uTimeUniform) uTimeUniform.value = globalTime;
        }
        try {
          if (composer) {
